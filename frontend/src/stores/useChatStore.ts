...
const transformBackendMessage = (payload: Message): Message => {
  return {
...
export const useChatStore = create<ChatState>((set, get) => ({
    ...
    client.onConnect = () => {
      ...
      client.subscribe("/user/queue/history", (message) => {
        try {
          ...
          const transformedMessages = history.map((msg: Message) => {
  ...
  loadMessages: async (channelId: string) => {
    ...
    const userId = useAuthStore.getState().user?.id || ""; // TODO: Get from useAuthStore
